...
## Static Build (django-distill)

This project is configured to export a fully static version of the site using django-distill.

Quick build:

- Run: `bash scripts/build_static.sh` to generate the static site into `distill_output/`.
- Add `--zip` to create a `static-site-<timestamp>.zip` archive.

What the script does:

- Creates/uses a virtualenv (default `.venv`).
- Installs `requirements.txt`.
- Runs `collectstatic` (writes to `distill_output/static`).
- Runs `distill-local` to render pages (writes to `distill_output/<lang>/...`).
- Copies `media/` to `distill_output/media` so the artifact is self-contained.

After completion, deploy the contents of `distill_output/` (or the zip) to any static host (Netlify, GitHub Pages, S3, etc.).
